...
public class TokenService : ITokenService
{
    ...
    public JwtSecurityToken GenerateAccessToken(IEnumerable<Claim> claims, IConfiguration config)
    {
        ...
        var tokenDescriptor = new SecurityTokenDescriptor
        {
            ...
            Expires = DateTime.UtcNow.AddMinutes(config.GetSection("JWT").GetValue<int>("TokenValidityInMinutes")),
    ...
    public ClaimsPrincipal GetPrincipal(string token, IConfiguration config)
    {
        var secretKey = config.GetSection("JWT").GetValue<string>("SecretKey") ?? throw new ArgumentException("Invalid key");
